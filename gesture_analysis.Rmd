...
```
![Snapshot of the Embodied Simulation of Population Dynamics.](ESPD.jpg)
...
hand = dat %>% transmute(norm_left_hand = 1 - (left_hand / max(left_hand)),
                         norm_right_hand = 1 - (right_hand / max(right_hand)))
```
```{r normalization2, fig.width=10, fig.height=4, fig.cap="(a) Elicited movement, (b) enacted movement Student 1, and (c) enacted movement Student 2."}
...
```{r paramTable}
fit %>% kable(caption = "HMM Model Selection", 
              digits = 2, format = "latex", align = "c")
colnames(responses) = c("mean_left_vel", "sd_left_vel", 
                        "mean_right_vel", "sd_right_vel")
responses %>% kable(caption = "Model Parameters", 
